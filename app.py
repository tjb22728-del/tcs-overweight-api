...
app = Flask(__name__)
CORS(app)  # Allow requests from your CodeSandbox dashboard

# ---------------------------------------------------------------------------
# Snowflake connection — set these as environment variables in Render.com
# ---------------------------------------------------------------------------
SF_USER     = os.environ.get("SF_USER",     "ZMDNZIEQEO_USER")
SF_PASSWORD = os.environ.get("SF_PASSWORD", "")           # set in Render
...
def overweights():
    try:
        ...
        # Build { product: [ { week_start, avg_overweight, avg_value, avg_target, count } ] }
        data = {}
        for week_start, product, avg_ow, avg_val, avg_tgt, count in rows:
